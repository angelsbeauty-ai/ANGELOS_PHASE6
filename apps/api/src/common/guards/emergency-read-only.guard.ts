...
export class EmergencyReadOnlyGuard implements CanActivate {
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Anchored to /workspaces/:id/... rather than a bare includes() so a route can't be spoofed by
    // an unrelated path segment, and the workspace/controls lookups fail closed (503) on error
    // rather than silently letting the action through.
