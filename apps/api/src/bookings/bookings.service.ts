...
export class BookingsService {
  ...
  private async transition(
    ...
    const { data: current, error: currentError } = await supabase.from('appointments').select('*').eq('workspace_id', workspaceId).eq('id', appointmentId).single();
    if (currentError || !current) throw new NotFoundException('Appointment not found');
    // Re-confirming/re-completing what already happened is a no-op, not an error -- a caller
    // retrying after a dropped response shouldn't see a failure for something that already
    // succeeded.
    if (current.status === status) return current;
    // Reject an illegal transition with a clear reason before touching the database. Without this
    // a cancelled or completed appointment could be moved back to confirmed -- and because
    // cancel() has already cancelled its automation jobs by then, the resurrected appointment
    // would silently lose its reminders.
    ...
    // Optimistic concurrency on top of the legality check above (same pattern as reschedule()):
    // by the time this runs the transition is known legal, so if zero rows match, another request
    // changed the row in between -- a genuine race, not an invalid transition.
