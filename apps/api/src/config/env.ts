...
function parseEnvironment(): AngelosEnvironment {
  ...
  return value as AngelosEnvironment;
}

function parseOrigins(raw: string | undefined, environment: AngelosEnvironment) {
...
export function validateRuntimeEnvironment(): RuntimeConfig {
  ...
  if (providerMode === 'openai') required('OPENAI_API_KEY');
