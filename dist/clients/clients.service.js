...
let ClientsService = exports.ClientsService = class ClientsService {
    ...
    async list(user, workspaceId, search) {
    ...
    async create(user, workspaceId, dto) {
        ...
        if (!displayName)
            throw new common_1.ConflictException('Client display name is required');
    ...
    async get(user, workspaceId, clientId) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
    ...
    async update(user, workspaceId, clientId, dto) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
        ...
        if (dto.source !== undefined)
            updates.source = dto.source?.trim() || null;
        if (dto.doNotAutoMessage !== undefined)
            updates.do_not_auto_message = dto.doNotAutoMessage;
        const { data, error } = await supabase.from('clients').update(updates).eq('workspace_id', workspaceId).eq('id', clientId).select('*').single();
        if (error || !data)
            throw new common_1.NotFoundException('Client not found');
        return data;
    }
    async addNote(user, workspaceId, clientId, dto) {
    ...
    async addTreatment(user, workspaceId, clientId, dto) {
        await this.assertClient(user, workspaceId, clientId);
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
    ...
    async addConsent(user, workspaceId, clientId, dto) {
        await this.assertClient(user, workspaceId, clientId);
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
    ...
    async addFollowup(user, workspaceId, clientId, dto) {
        await this.assertClient(user, workspaceId, clientId);
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
    ...
    async assertUniqueContact(supabase, workspaceId, email, phone, excludeId) {
        for (const [field, value] of [['email', email], ['phone', phone]]) {
            ...
            if (error)
                throw new common_1.InternalServerErrorException(error.message);
            if (data?.length)
                throw new common_1.ConflictException('A client with matching contact information already exists');
        }
    }
    async assertClient(user, workspaceId, clientId) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
