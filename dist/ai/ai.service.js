...
let AiService = exports.AiService = AiService_1 = class AiService {
    ...
    async getProfile(user, workspaceId) {
        ...
        if (createError || !created) {
            throw new common_1.InternalServerErrorException(createError?.message ?? 'Could not create assistant profile');
        }
        const roleKeys = [
            'personal_assistant',
        ...
        const { error: roleError } = await supabase.from('ai_assistant_roles').upsert(roleKeys.map((role_key) => ({
            workspace_id: workspaceId,
            role_key,
            enabled: true,
        ...
        const { data: seededRoles, error: seededRolesError } = await supabase
            ...
            .order('role_key');
        if (seededRolesError)
            throw new common_1.InternalServerErrorException(seededRolesError.message);
        return { profile: created, roles: seededRoles ?? [] };
    }
    async updateProfile(user, workspaceId, dto) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
        const updates = { updated_at: new Date().toISOString() };
        if (dto.displayName !== undefined)
            updates.display_name = dto.displayName.trim();
        if (dto.avatarKey !== undefined)
            updates.avatar_key = dto.avatarKey;
        if (dto.personalityPrompt !== undefined)
            updates.personality_prompt = dto.personalityPrompt;
        if (dto.primaryLanguage !== undefined)
            updates.primary_language = dto.primaryLanguage;
        if (dto.tone !== undefined)
            updates.tone = dto.tone;
        if (dto.responseLength !== undefined)
            updates.response_length = dto.responseLength;
        if (dto.proactivity !== undefined)
            updates.proactivity = dto.proactivity;
        if (dto.floatingButtonMode !== undefined)
            updates.floating_button_mode = dto.floatingButtonMode;
        if (dto.guidanceQuestionsEnabled !== undefined)
            updates.guidance_questions_enabled = dto.guidanceQuestionsEnabled;
        if (dto.explainRecommendations !== undefined)
            updates.explain_recommendations = dto.explainRecommendations;
        if (typeof updates.display_name === 'string' && !updates.display_name) {
            throw new common_1.BadRequestException('Assistant name cannot be empty');
        }
    ...
    async updateRoles(user, workspaceId, dto) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
    ...
    async createConversation(user, workspaceId, dto) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
        const { data, error } = await supabase
    ...
    async listMessages(user, workspaceId, conversationId) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
        const { data, error } = await supabase
            .from('ai_messages')
    ...
    async sendMessage(user, workspaceId, conversationId, dto) {
        ...
        if (userMessageError)
            throw new common_1.InternalServerErrorException(userMessageError.message);
        ...
        if (profileResult.error || !profileResult.data)
            throw new common_1.NotFoundException('Assistant profile not found');
        if (rolesResult.error)
            throw new common_1.InternalServerErrorException(rolesResult.error.message);
        if (memoryResult.error)
            throw new common_1.InternalServerErrorException(memoryResult.error.message);
        if (historyResult.error)
            throw new common_1.InternalServerErrorException(historyResult.error.message);
        ...
        const instructions = (0, angelos_operating_contract_1.buildOperatingInstructions)({
            workspaceName: workspaceResult.data.name,
            profile: profileResult.data,
            roles: (rolesResult.data ?? []),
            approvedMemory: (memoryResult.data ?? []).map((row) => row.content),
            context: dto.context,
            contextFacts
        });
        ...
        const plannedAction = (0, action_planner_1.planSafeAssistantAction)(message);
        let actionRun = null;
        if (plannedAction) {
            const serviceSupabase = (0, supabase_1.createServiceSupabaseClient)();
        ...
        const serviceSupabaseForMessage = (0, supabase_1.createServiceSupabaseClient)();
        ...
        if (assistantMessageError)
            throw new common_1.InternalServerErrorException(assistantMessageError.message);
        return { message: assistantMessage, action: actionRun };
    }
    async approveAction(user, workspaceId, actionId) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
        ...
        if (controls?.emergency_read_only)
            throw new common_1.BadRequestException('AngelOS is in emergency read-only mode. AI can still chat, but it cannot change business data.');
        if (controls?.pause_ai_actions)
            throw new common_1.BadRequestException('AI actions are paused by the workspace owner.');
        const { data: action, error } = await serviceSupabase
            ...
            .single();
        if (error || !action)
            throw new common_1.NotFoundException('Proposed action not found');
        ...
        try {
            ...
            if (action.action_key === 'update_assistant_name') {
                ...
                if (!displayName)
                    throw new Error('Missing assistant display name');
    ...
    async cancelAction(user, workspaceId, actionId) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
        await this.assertWorkspaceAccess(supabase, workspaceId);
        const serviceSupabase = (0, supabase_1.createServiceSupabaseClient)();
        const { data, error } = await serviceSupabase
    ...
    async proposeMemory(user, workspaceId, dto) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
        const content = dto.content?.trim();
        if (!content)
            throw new common_1.BadRequestException('Memory content is required');
        const { data, error } = await supabase
            .from('ai_memory_items')
            .insert({
            workspace_id: workspaceId,
    ...
    async listMemory(user, workspaceId) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
        const { data, error } = await supabase
            .from('ai_memory_items')
    ...
    async sendVoice(_user, _workspaceId, dto) {
        try {
            const base64 = dto.audioBase64.trim();
            if (!base64 || base64.length < 100) {
                return { error: 'Audio too short', status: 400 };
            }
            this.logger.warn('ai-agent-voice Edge Function is not deployed — transcription pending. ' +
                `audio bytes received: ${base64.length}`);
            return {
                transcript: '',
                reply: '🎤 Voice received. Transcription is not wired up on the server yet — ' +
    ...
    async loadAuthorizedContextFacts(supabase, workspaceId, context) {
        if (!context?.entityId)
            return [];
        if (context.entityType === 'client') {
            ...
            for (const treatment of treatments ?? [])
                facts.push(`Treatment ${treatment.performed_at}: ${treatment.service_name}; stage=${treatment.stage}; technique=${treatment.technique ?? 'not recorded'}; notes=${treatment.notes ?? 'none'}`);
            for (const note of notes ?? [])
                facts.push(`Client note (${note.note_type}, ${note.created_at}): ${note.content}`);
            return facts;
        }
        if (context.entityType === 'content_post') {
            ...
            for (const item of post.media ?? [])
                facts.push(`Media ${item.position}: ${item.asset?.original_filename ?? 'unknown'}; type=${item.asset?.media_type ?? 'unknown'}; permission=${item.asset?.marketing_permission ?? 'unknown'}; contentStatus=${item.asset?.content_status ?? 'unknown'}`);
            for (const variant of post.variants ?? [])
                facts.push(`${variant.platform} variant: format=${variant.format}; status=${variant.status}; hook=${variant.hook ?? 'none'}; caption=${variant.caption}; CTA=${variant.cta ?? 'none'}; scheduled=${variant.scheduled_for ?? 'not scheduled'}`);
            return facts;
        }
        if (context.entityType === 'message_thread') {
            ...
            for (const item of (messages ?? []).reverse())
                facts.push(`${item.direction === 'inbound' ? 'Client' : 'Business'} message (${item.created_at}, ${item.status}${item.sensitive ? ', sensitive' : ''}): ${item.body}`);
            return facts;
        }
        return [];
    }
    ...
    async verifyAction(user, workspaceId, actionKey, input) {
        const supabase = (0, supabase_1.createUserSupabaseClient)(user.accessToken);
